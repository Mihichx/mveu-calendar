...
@app.route('/')
def home():
    return 'Бот МВЕУ работает! Ссылка для календаря: <a href="/calendar.ics">/calendar.ics</a>'
            ...
            for row in rows:
                ...
                try:
                    ...
                    if '-' in time_text and subject and current_date_str:
                        ...
                        # Уведомление только на первую подходящую пару дня
                        if current_date_str not in days_with_alarm:
                            # 1. За 1 час: если есть цифры (номер кабинета) или Спортзал
                            ...
                            # 2. За 10 минут: Вебинары или (базовый) Код Будущего
                            elif "вебинар" in room_l or "авторизируйтесь" in room_l or "базовый" in subj_l:
                                alarm = Alarm()
                                alarm.add('action', 'DISPLAY')
                                alarm.add('trigger', timedelta(minutes=-10))
                                alarm.add('description', "Скоро начало")
                                e.add_component(alarm)
                                days_with_alarm.add(current_date_str)
